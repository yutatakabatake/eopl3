...
; CPS-OUTのASTからソースコードに変換する逆コンパイラ
...
; decompiler ----------------------------------------------------------------------------
(define decompile-cps-program
  (lambda (pgm)
    (cases cps-out-program pgm
      (cps-a-program (exp1)
                     (decompile-cps-tfexp exp1)))))

(define decompile-cps-tfexp
  (lambda (exp)
    (cases tfexp exp
      (simple-exp->exp (simple-exp1)
                       (decompile-cps-simple-exp simple-exp1))
      (cps-call-exp (rator rands)
                    (let ((ratorc (decompile-cps-simple-exp rator))
                          (randsc (map decompile-cps-simple-exp rands)))
                      (cons ratorc randsc)))
      (cps-if-exp (simple1 body1 body2)
                  (let ((simple1c (decompile-cps-simple-exp simple1))
                        (body1c (decompile-cps-tfexp body1))
                        (body2c (decompile-cps-tfexp body2)))
                    `(if ,simple1c ,body1c ,body2c)))
      ; let
      ; letrec
      (else decompile-cps-simple-exp exp))))





(define decompile-cps-simple-exp
  (lambda (exp)
    (cases simple-exp exp
      ...
      (cps-diff-exp (simple1 simple2)
                    (let ((simple1c (decompile-cps-simple-exp simple1))
                          (simple2c (decompile-cps-simple-exp simple2)))
                      `(- ,simple1c ,simple2c)))
      (cps-zero?-exp (simple1)
                     (let ((simple1c (decompile-cps-simple-exp simple1)))
                       `(zero? ,simple1c)))
      (cps-proc-exp (vars body)
                    (let ((bodyc (decompile-cps-tfexp body)))
                      `(lambda ,vars ,bodyc)))
      (cps-sum-exp (simple-exps)
                   (let ((simple-expsc (map decompile-cps-simple-exp simple-exps)))
                     (cons '+ simple-expsc)))
      (else (display 'err)))))
...
(define decompile
  (lambda (string)
    (decompile-cps-program (translate string))))
