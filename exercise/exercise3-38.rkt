  ...
  (lambda (exp senv)
    (cases expression exp
      ...
      (cond-exp (conditions bodies)
                (let ([nameless-conditions (map (lambda (cond) (translation-of cond senv)) conditions)]
                      [nameless-bodies (map (lambda (body) (translation-of body senv)) bodies)])
                  (cond-exp nameless-conditions nameless-bodies)))
      ...
      (cond-exp (conditions bodies)
                (if (null? conditions)
                    ...
                    (let ([condition (car conditions)]
                          [body (car bodies)])
                      (if (expval->bool (value-of condition nameless-env))
                          (value-of body nameless-env)
                          (value-of (cond-exp (cdr conditions) (cdr bodies)) nameless-env)))))
