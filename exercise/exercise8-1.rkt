#lang eopl
; SIMPLE-MODULEに重複するモジュール名を許さないチェッカーを追加する
...
(define module-definition->m-name
  (lambda (m-defn)
...
(define definition->var-name
  (lambda (defn)
    (cases definition defn
      (val-defn (var-name exp1)
                var-name))))
  ...
  (lambda (pgm)
    (cases program pgm
      (a-program (module-defns body)
                 (if (car (have-duplicate? module-defns))
                     (eopl:error 'module-name-chcker "Duplicate module name: ~s" (car (reverse (have-duplicate? module-defns))))
                     (type-of body
...
(define have-duplicate?
  (lambda (module-defns)
    (if (null? module-defns)
        '(#f)
        (let ((var-names (map (lambda (module-defn) (module-definition->m-name module-defn)) module-defns)))
...
(define check-duplicate
  (lambda (lst)
    (if (null? lst)
        '(#f)
        (let ((sym (car lst)))
          (if (member sym (cdr lst))
              (list #t sym)
              (check-duplicate (cdr lst)))))))
