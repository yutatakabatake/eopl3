...
(define-datatype environment environment?
  ...
  (extend-env
   (var identifier?)
   (val denval?) ;denoted value = reference
   (env environment?))
  (extend-env-rec
...
(define value-of
  (lambda (exp env)
    (cases expression exp
      ...
      (var-exp (var)
               (let ((denval1 (apply-env env var)))
                 (cases denval denval1
                   (den-ref (ref)
                            (deref ref))
                   (den-expval (val)
                               val))))
      ...
      (let-exp (var exp1 body)
               (let ((val1 (value-of exp1 env)))
                 (value-of body
                           (extend-env var (den-expval val1) env))))
      (proc-exp (var body)
                  ...
                  (let ((denval1 (apply-env env var)))
                    (begin
                      (setref!
                       (cases denval denval1
                         (den-ref (ref) ref)
                         (den-expval (_) (eopl:error 'assign-exp "~s is not reference" denval1)))
                       (value-of exp1 env))
                      (num-val 27))))
