...
; listof型を拡張
; list-exp, cons-exp, null-exp, empty-list-expを拡張
  ...
  (lambda (ty)
    (cases type ty
      (list-type (ty)
                 ty)
      (else
       (report-type-extractor-error 'list-type ty)))))
    ...
    (cases expression exp
      ...
      (list-exp (exp1 exps)
                (let ((ty1 (type-of exp1 tenv))
                  ...
                  (begin
                    (map (lambda (ty) (check-equal-type! ty1 ty exp1)) tys)
                    (list-type ty1))))
      (cons-exp (exp1 exp2)
                (let ((ty1 (type-of exp1 tenv))
                      (ty2 (type-of exp2 tenv)))
                  (check-equal-type! ty2 (list-type ty1) exp)
                  (list-type ty1)))
      (null-exp (exp1)
                (let ((ty1 (type-of exp1 tenv)))
                  (check-equal-type! ty1 (list-type (list-type->ty ty1)) exp1)
                  (bool-type)))
      (emptylist-exp (ty1)
                     (list-type ty1)))))
