...
(define value-of
  (lambda (exp env)
    (cases expression exp
      ...
      (proc-exp (var body)
                (let ((free-vars (find-free body (list var))))
                  (let ((free-vars-vals (map (lambda (var) (apply-env env var)) free-vars)))
                    (proc-val (procedure var body (make-proc-env free-vars free-vars-vals (empty-env)))))))
...
(define make-proc-env
  (lambda (vars vals env)
    ...
    (if (null? vars)
        env
        (make-proc-env (cdr vars) (cdr vals) (extend-env (car vars) (car vals) env)))))
