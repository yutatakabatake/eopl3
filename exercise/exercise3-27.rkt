      ...
      (procedure (vars body saved-env trace?)
                 (if trace?
                     (if (null? vars)
                         ...
                         (begin
                           (eopl:printf "enter: ~a = ~a\n" (car vars) (car arg))
                           (let ((var (car vars))
                                 (rest-var (cdr vars))
                                 (val (car arg))
                                 (rest-val (cdr arg)))
                             (eopl:printf "exting: result = ~a\n" (apply-procedure (procedure rest-var body (extend-env var val saved-env) #t) rest-val)))))
                     (if (null? vars)
                         (value-of body saved-env)
                         (let ((var (car vars))
                               (rest-var (cdr vars))
                               (val (car arg))
                               (rest-val (cdr arg)))
                           (apply-procedure (procedure rest-var body (extend-env var val saved-env) #f) rest-val))))))))
  ...
  (lambda (exp env)
    (cases expression exp
      ...
      (letproc-exp (p-name p-var p-body body)
                   (let ((val1 (proc-val (procedure p-var p-body env #f))))
