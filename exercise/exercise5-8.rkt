  ...
  (lambda (cont val)
    (cases continuation cont
      ...
      (rator-cont (rands env cont)
                  (value-of/k (car rands) env (randfst-cont val (cdr rands) env cont)))

      (randfst-cont (val1 rands env cont)
                    (if (null? rands)
                        (let ((proc1 (expval->proc val1)))
                          (apply-procedure/k proc1 (list val) cont))
                        (value-of/k (car rands) env
                                    (randrst-cont (expval->proc val1) (list val) (cdr rands) env cont))))
      (randrst-cont (proc1 vals rands env cont)
                    (if (null? rands)
                        (apply-procedure/k proc1 (append vals (list val)) cont)
                        (value-of/k (car rands) env
                                    (randrst-cont proc1 (append vals (list val)) (cdr rands) env cont))))
