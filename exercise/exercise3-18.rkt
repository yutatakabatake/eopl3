...
;cdrがlist-valのlistを返すようになるのを修正する
;expval->cdr : ExpVal → Listof(ExpVal)
(define expval->cdr
  (lambda (val)
    (cases expval val
      (list-val (elems)
    ...
    (cases expression exp
      ...
      (if-exp (exp1 exp2 exp3)
              (let ((val1 (value-of-bool-exp exp1 env)))
                (if (bool-val->bool val1)
                    (value-of exp2 env)
                    (value-of exp3 env))))
      (let-exp (vars exps body)
               (let ((vals (map (lambda (exp) (value-of exp env)) exps))) ;先に評価してから前の環境で評価して環境に登録する
      ...
      (cond-exp (preds clauses)
                (let ((pred (car preds))
                      (clause (car clauses)))
                  (if (bool-val->bool (value-of-bool-exp pred env))
                      (value-of clause env)
                      (value-of (cond-exp (cdr preds) (cdr clauses)) env))))
      (print-exp (exp1)
                 (display (expval->num (value-of exp1 env)))
                 (newline)
                 (num-val 1))
      (let*-exp (vars exps body)
                (if (or (null? vars)
                    ...
                    (let ((var (car vars))
                      ...
                      (let ((val (value-of exp env)))
                        (value-of (let*-exp rest-vars rest-exps body) (extend-env var val env))))))
      ...
      (unpack-exp (vars exp1 body)
                  (cases expression exp1
                    ; (cons-exp (fst rst)
                    ;           (let ((val1 (value-of exp1 env)))
                    ;             (let ((fst-val (expval->car val1))
                    ;                   (rst-val (expval->cdr val1)))
                    ;               (extend-env))))
                    (list-exp (exps)
                              (let ((vals (map (lambda (exp) (value-of exp env)) exps)))
                                (value-of body (extend-env-list vars vals env))))
                    (else
                     (report-not-list exp1))))
      )))
...
(define report-not-list
  (lambda (val)
    (eopl:error 'value-of "Not list: ~s" val)))
