  ...
  (lambda (exp env)
    (cases expression exp
      ...
      (let-exp (var exp1 body)
               (let ((ref1 (value-of-operand exp1 env)))
                 (value-of body (extend-env var ref1 env))))
      (proc-exp (var body)
...
(define scan&parse
  (sllgen:make-string-parser scanner-spec-ref-name grammar-ref-name))

(define repl
  (sllgen:make-rep-loop "--> "
                        (lambda (pgm) (value-of-program pgm))
                        (sllgen:make-stream-parser scanner-spec-ref-name grammar-ref-name)))
...
(define terminate-test
  "letrec infineteLoop (x) = (infineteLoop -(x,-1))
    in let f = proc (z) 11
        in (f (infineteLoop 0))")