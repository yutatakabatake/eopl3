...
(define find-free
  (lambda (exp b-vars)
    (cases expression exp
      ...
      (diff-exp (exp1 exp2)
                (append
                 (find-free exp1 b-vars)
                 (find-free exp2 b-vars)))
      (zero?-exp (exp1)
                 (find-free exp1 b-vars))
      (if-exp (exp1 exp2 exp3)
              (append
               (find-free exp1 b-vars)
               (find-free exp2 b-vars)
               (find-free exp3 b-vars)))
      (let-exp (var1 exp1 body)
               (append (find-free exp1 b-vars)
                       (find-free body (cons var1 b-vars))))
      (proc-exp (var1 body)
                (find-free body (cons var1 b-vars)))
      (call-exp (rator rand)
                (append (find-free rator b-vars)
                        (find-free rand b-vars)))
      (nameless-var-exp (n) '())
      (nameless-let-exp (exp1 body)
                        (append (find-free exp1 b-vars)
                                (find-free body b-vars)))
      (nameless-proc-exp (body)
                         (find-free body b-vars)))))

(define make-proc-env
  (lambda (vars env)
    (if (null? vars)
        env
        (make-proc-env (cdr vars) (extend-senv (car vars) env)))))
...
(define value-of
  (lambda (exp nameless-env)
    ...
    (cases expression exp
      ...
      (nameless-proc-exp (body)
                         (proc-val
                          (procedure body (trim-env body nameless-env))))
      (else
...
(define trim-env
  (lambda (exp nameless-env)
    (cases expression exp
      ...
      (diff-exp (exp1 exp2)
                (trim-env exp2 (trim-env exp1 nameless-env)))
      (zero?-exp (exp1)
                 (trim-env exp1 nameless-env))
      (if-exp (exp1 exp2 exp3)
              (trim-env exp3
                        (trim-env exp2
                                  (trim-env exp1 nameless-env))))
      (call-exp (rator rand)
                (trim-env rand (trim-env rator nameless-env)))
      (nameless-var-exp (n)
                        (extend-nameless-env (apply-nameless-env nameless-env (- n 1))
                                             (empty-nameless-env)))
      (nameless-let-exp (exp1 body)
                        (trim-env body (trim-env exp1 nameless-env)))
      (nameless-proc-exp (body)
                         (trim-env body nameless-env))
      (else
       (report-invalid-translated-expression exp)))))
